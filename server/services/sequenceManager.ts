...
export async function createSequence(input: CreateSequenceInput): Promise<{
  ...
  // Enforce one active sequence per contact only after all inputs are known valid.
...
export async function processDueSteps(userId: string): Promise<number> {
  ...
  for (const step of dueSteps) {
    try {
      ...
      // Leave the step pending so a transient action-queue failure can be retried.
      continue;
    }
